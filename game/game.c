...
void progress_print(WINDOW *win, GameState *state) {
    ...
    mvwprintw(win, 7, 2, "Dice 1: ");
    mvwprintw(win, 7, 14, "Dice 2: ");
    mvwprintw(win, 8, 2, "Total: ");
    if (player == 0)
        player_hint(win);
    wrefresh(win);
}
...
void endTurn(WINDOW *win, GameState *state) {
    state->current_turn++;
    state->current_player = state->current_turn % (state->number_of_players);
    progress_print(win, state);
}
...
void menu(WINDOW *win, GameState *state) {
    ...
    while ((ch = getch()) != 'q') {
        ...
        case 'e':
            endTurn(win, state);
            clear_hint(win);
            for (int i = 0; i < (state->number_of_players - 1); i++)
                npc_act(win, state);
            dice_rolled = 0;
            break;
        default:
...
void npc_act(WINDOW *win, GameState *state) {
    ...
    startTurn(win, state);
    sleep(1);
    // check resource then do something
    // sleep(1);
    endTurn(win, state);
}